...
function g { $path = (eagle goto $args); if ($path) { Set-Location $path } }
"#;

fn build() -> Command {
	Command::new("init")
		.about("Install shell integrations (PowerShell profile)")
		.long_about(
			"Appends the `g` function to your PowerShell profile so that\n\
...
fn run(_: &ArgMatches, _: &Context) -> anyhow::Result<()> {
	let profile_path = powershell_profile()?;
	...
	println!(
		"Added `g` function to {}\nRestart your shell or run `. $PROFILE` to apply.",
		profile_path.display()
	);
	Ok(())
}

fn powershell_profile() -> anyhow::Result<std::path::PathBuf> {
